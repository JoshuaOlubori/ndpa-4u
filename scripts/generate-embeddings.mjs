...
if (!GEMINI_API_KEY || !DATABASE_URL) {
  console.warn('[embeddings] GEMINI_API_KEY or DATABASE_URL missing — skipping.')
  process.exit(0)
}
...
function cleanMarkdown(raw) {
  ...
  return raw.split('\n').filter(line => !/^:{2,}/.test(line.trim())).join('\n')
}
...
function chunkMarkdown(markdown, maxChars = 1200, overlap = 150) {
  ...
  for (const section of sections) {
    ...
    while (start < text.length) {
      ...
      chunks.push({ heading: section.heading, text: text.slice(start, end).trim() })
...
async function main() {
  ...
  for (const file of files) {
    ...
    const [existing] = await sql`SELECT hash FROM doc_file_hashes WHERE path = ${routePath}`
    ...
    embeddedCount++
    console.log(`[embeddings] Re-embedded ${routePath} (${chunks.length} chunks)`)
  }
  ...
  console.log(`[embeddings] Done. ${embeddedCount} files re-embedded, ${skippedCount} unchanged.`)
}
